...
docker run --rm -p 8080:8080 --env DOWNLOAD_RETRY_COUNT=1 \
--env PARKANDRIDE_URL=$PARKANDRIDE_URL --env TICKET_SALES_URL=$TICKET_SALES_URL \
--env HSL_OTP_URL=$HSL_OTP_URL --env FINLAND_OTP_URL=$FINLAND_OTP_URL --env WALTTI_OTP_URL=$WALTTI_OTP_URL \
